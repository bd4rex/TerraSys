...
if ($LASTEXITCODE -ne 0) {
  throw "Docker is not running or unavailable. Run ./terrasys.sh start (Linux) or start-terrasys.cmd (Windows) first."
}
...
$web = Invoke-WebRequest -UseBasicParsing -Uri "$baseUrl/healthz"
$checks.Web = $web.StatusCode

$api = Invoke-RestMethod -Uri "$baseUrl/api/health"
$checks.Api = $api.status
$checks.PostGIS = $api.postgis

$catalog = Invoke-RestMethod -Uri "$baseUrl/martin/catalog"
$checks.MartinSources = @($catalog.tiles.PSObject.Properties).Count

$packStatus = Invoke-RestMethod -Uri "$baseUrl/api/map-packs"
...
foreach ($pack in $installedPacks) {
  $headers = curl.exe -sS -I -H "Range: bytes=0-1023" "$baseUrl$($pack.url)"
...
try {
  $resources = Invoke-RestMethod -Uri "$baseUrl/api/resources?cached=true" -TimeoutSec 10
}
catch {
  # A first start may not have produced the persistent inventory yet.
  $resources = Invoke-RestMethod -Uri "$baseUrl/api/resources" -TimeoutSec 90
}
...
$overviewVectorRequest = [System.Net.HttpWebRequest]::Create("$baseUrl/assets/overview/world-overview.pmtiles")
...
$capabilities = Invoke-RestMethod -Uri "$baseUrl/api/capabilities" -TimeoutSec 20
...
if ($capabilities.source) {
  ...
  $wiki = Invoke-WebRequest -UseBasicParsing -Uri "$baseUrl/wiki/catalog/v2/entries?count=-1" -TimeoutSec 20
