...
- MCP 改为标准 NDJSON；GPX 导入采用单事务；改用测地距离，并通过迁移 008 修正已存轨迹里程。
- 地图完整候选版本、激活日志与失败恢复；个人数据库/媒体恢复检查点及相关进程协调。
- 离线包 schema v5 统一项目文件与镜像清单，补齐构建配置和 Linux 入口，保护地图快照并支持重复刷新。
- 路线结果绑定输入快照，取消过期请求，修复慢信源与空地图目录；AIS 按需启动，TTL/LRU 缓存限制容量。
- 所有测试 profile 加入确定性失败路径回归，并增加独立 PostGIS CI job。见[可靠性升级说明](docs/RELIABILITY_FIXES.zh-CN.md)。
