...
部分公开区域提取文件会在裁切或过滤边界省略外部引用。大陆省份使用 `complete_ways`，而直接下载、区域构建和共享源合并都会解析 Osmium 的实际缺失计数，并与来源配置中的有限上限比较；无法识别的错误或超过上限都会阻止启用。

当前 OpenStreetMap Korea 的朝鲜“非军事要素”提取文件在完整 PBF 扫描后报告 18,086 个缺失道路节点，韩国对应文件为 0。因此朝鲜来源显式配置 20,000 上限，不会静默放行。共享能力构建把各已验证输入的上限相加，并在 manifest 中记录合并后的实际计数；超限仍按失败关闭处理。大陆省级提取每个输入最多接受 100 个可解析的提供方边界缺口。
